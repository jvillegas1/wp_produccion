...
<channel>
 ...
 <cms:contentload collector="singleFile" param="%(opencms.uri)" >
    ...
    <c:choose>
  <c:when test='${pagina == "blog"}'>
 <nt:news-list newstype="bloggers" order="user-modification-date" size="${cantidad}" >
      <item>
     ...
     <pubDate><nt:last-modified format="dd MMMM yyyy"/></pubDate>
	 ...
	 <biografia>
	 	<nt:user username="${internalUser}">
	 	<![CDATA[<nt:conditional-include oncondition="${ntuser.infoextra['USER_DESCRIPTION'] != null && ntuser.infoextra['USER_DESCRIPTION'] != ''}">
			     ${ntuser.infoextra['USER_DESCRIPTION']}
  ...
  <c:otherwise>
<nt:news-list newstype="bloggers" order="user-modification-date" size="20" >
   <item>
     <title><![CDATA[<nt:title/>]]></title>
     <link><%=request.getScheme()+"://"+request.getServerName()%><nt:link/></link>
     <description><![CDATA[<nt:sub-title/>]]></description>
