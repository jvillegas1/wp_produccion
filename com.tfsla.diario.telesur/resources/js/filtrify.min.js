...
(function($, window, document, undefined) {
	...
	var may = MaysPrimera(f);
	function MaysPrimera(f){ 
	    return f.charAt(0).toUpperCase() + f.slice(1); 
	}
        var html, t, tag, tags = [];
        html = "<li class='ft-field'>" + "<span class='ft-label ft-opened'>" + may + "</span>" + "<div class='ft-panel ft-opened'>" + "<ul class='ft-selected' style='display:none;'></ul>" + "<ul class='ft-tags'>";
    ...
    Filtrify.prototype.moveHighlight = function(f, direction) {
        if (this._menu[f].highlight.length) {
            var method = direction === "down" ? "nextAll" : "prevAll", next = this._menu[f].highlight[method](":visible:first");
            if (next.length) {
                this.clearHighlight(f);
    ...
    Filtrify.prototype.showMatch = function(row, match) {
        if (this.options.hide) {
            ...
            if (match) {
                if (hidden)
                    this._items[row].className = this._items[row].className.replace(/ft-hidden/g, "");
            } else {
                if (!hidden)
                    this._items[row].className = this._items[row].className + " ft-hidden";
            }
            ;
        }
        ;
    ...
    Filtrify.prototype.updatePanel = function(f) {
        var t = 0, tag, tags = this._menu[f].tags.children().removeClass("ft-hidden");
