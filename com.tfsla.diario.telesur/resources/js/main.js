jQuery(document).ready(function($){
	...
	var ancla           = $('.anclaFotoFull');
	var positionPhotos  = $('#espFotos').position();
	...
	function updateSidebarPosition() {
		var articlesWrapperTop = articlesWrapper.offset().top,
			...
			positionPhotosTop = positionPhotos.top,
			positionFooterTop = positionFooter.top,
			windowWidth = $(window).width();
						
			
		if( scrollTop >= (positionFooterTop + 1124) && windowWidth <= 1400) {
		
		console.log("footer: " + positionFooterTop + " scroll: "+scrollTop);
			aside.removeClass('fixed').css('top', (positionFooterTop + 859) + 'px');
		...
		if( scrollTop < articlesWrapperTop) {
			...
			ancla.removeClass('anclaFixed').attr('style', '');

		} else if( scrollTop >= (positionPhotosTop-300) && windowWidth <= 1400) {
			 ancla.removeClass('anclaFixed').css('top', (positionPhotosTop - 859) + 'px');
			 
			
		} else if( scrollTop >= articlesWrapperTop ) {
			aside.addClass('fixed').attr('style', '');
			ancla.addClass('anclaFixed').attr('style', '');	
			...
			if( aside.hasClass('fixed') )
			 ...
			 ancla.removeClass('anclaFixed').css('top', articlesWrapperHeight + articlePaddingTop - windowHeight + 'px');
			 aside.removeClass('fixed').css('top', positionFooterTop + 'px');
