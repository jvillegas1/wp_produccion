...
//Country
List<CmsCategory> countries = cService.readCategories(action.getCmsObject(),"/SubSecciones/country",false,referencePath);
...
<div class="col">
	<div class="rowInt ">
		<div class="headline cont">Titulares por país</div>
	    	
	    	<c:set var="i" value="0" />
	    	<% for (CmsCategory country : countries){ 
	    	String path = country.getPath().replaceAll("SubSecciones/","") ;
	    	%>
			        ...
			        <div class="headlinecol listhead">
			            <ul>
                		    	<li class="forum <% if (country.getPath().replaceAll("SubSecciones/","").equals(subSeccion+"/")) { %> headsel <% } %>">
                		    		<a href="/<%=path %>" class="her" alt="<%=country.getDescription()%>" title="<%=country.getDescription()%>"><%=country.getDescription()%></a>
                		    	</li>
			            </ul>
			        </div>			
				<div class="headlinecol listhead ">
