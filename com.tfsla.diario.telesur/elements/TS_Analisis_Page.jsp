...
<%--el orden que tenia antes. por si lo piden => order="<%=display_search%>" order="user-modification-date desc"--%>
	<nt:news-list section="analisis" order="user-modification-date desc" advancedfilter="<%=query %>" author="<%=filter_autor%>" size="<%=size%>" page="<%=pagina%>" zone="normal" onmainpage="section" searchindex="<%=index%>">	
			<div class="contblog" style="max-width: 530px;" itemscope itemtype="http://schema.org/Article">
			...
			<nt:conditional-include oncondition="${newslist.size == 4}">	
				...
				<c:if test="${hayMas == 'true'}" >
					...
					<div class="cont" id="paginador" > 
						<a class="btnrepsm" href="javascript:cargarMasNotas('<%=pagina+1%>','<%=size%>','<%=display_search%>','<%=filter_autor%>','<%=buscar_search%>');"><img src="/arte/down-arrow.png" width="24" height="13" alt="" />Ver más análisis</a>
					</div>	
				</c:if>
			</nt:conditional-include> 			 
		</nt:conditional-include> 
