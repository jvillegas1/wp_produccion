...
<!--TIRAAAA <%=category%> <%=pagina%> -->
<nt:news-list order="user-modification-date desc" category="<%=category%>" size="<%=size%>" page="<%=pagina%>">
	...
	<nt:conditional-include onposition="last">
		...
		<nt:conditional-include oncondition="${ (newslist.size == 11 && newslist.page == 1) || (newslist.size == 5 &&  newslist.page != 1) }">
			<nt:news-list order="user-modification-date desc" category="<%=category%>" size="<%=size%>" page="<%=pagina+1%>">
			...
			<c:if test="${hayMas == 'true'}" >
				...
				<div class="btnarg" id="paginador" > 
					<a class="subscrip" href="javascript:cargarMasNotas('<%=proxPag %>','<%=category%>', '<%=title%>', '5');">Más titulares de <%=title%></a>
				</div>	
			</c:if>	
		</nt:conditional-include> 
