...
if (request.getParameter("apodo") != null)
{
	...
	if(tipoPerfil.equals("wp")){
		username = request.getParameter("apodo");//.replaceAll("_"," ");
	}else{
		String apodoFrm = request.getParameter("apodo");//.replaceAll("_"," ");
	...
	<nt:news-list advancedfilter="<%=query %>" size="10" page="<%=pagina %>" state="pendientePublicacion" order="user-modification-date" >
		...
		<nt:conditional-include oncondition="${newslist.absoluteposition == 1 }">
			<div class="rowInt">
			        ...
			        <div class="headers blue">Notas Pendientes de Aprobación </div>
			        <div class="cont">
		...
		<nt:conditional-include oncondition="${newslist.absoluteposition == 1 }">
			...
			<script type="text/javascript">
				function cargarNotasPendientes(_proxPag, _userName){
					...
					$.post("/system/modules/com.tfsla.diario.telesur/elements/webusers/TS_Profile_NewsList_Pending.jsp",
