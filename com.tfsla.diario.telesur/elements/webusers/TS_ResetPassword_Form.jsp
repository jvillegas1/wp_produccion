...
if(request.getParameter("registration_email") != null){
	...
	if (!showCapchaError) { 
		...
		errorMessage = "Verifique los campos resaltados.";
	}	
}
%>
...
<c:if test='${RecuperaPassMailSent == "true"}' >
	...
	<div class="cont">
		<div class="aprobado">
			<div class="txtaviso"> Se ha enviado un e-mail con información de tu cuenta para que vuelvas a ingresar a teleSUR. </div>
		</div>
	</div>
	
	<div class="cont">
  		<div class="btnConf">
       			<a href="/index.html">Volver a teleSUR</a>
    		</div>
	</div>
...
<c:if test='${RecuperaPassMailSent == "false"}'>
	...
	<div class="col errorRecu" id="recupera">
		<div class="cont titreg"> Recuperar contraseña </div>
		<form id="cal_registration_form" name="cal_registration_form" method="post" action="<%= currentUri %>"  class="form-elements" >
		<div class="col-mid">
			<div class="cont">
					<c:if test='${error == "true"}'>
						<input type="text" class="mal" value="<%=valueEmail %>" name="registration_email" placeholder="Ingresa tu Email*">
					</c:if>
					<c:if test='${error != "true"}'>
						<input type="text" value="<%=valueEmail %>" name="registration_email" placeholder="Ingresa tu Email*">
					</c:if>
		...
		<p style="border-bottom: 1px solid #CCCCCC;float: left;margin-bottom: 25px;width: 100%;"></p>
					...
					<input id="subscribe" type="submit" value="Recuperar contraseña" name="submit" style="float:right;max-width:none">
		</form>
	</div>
</c:if>