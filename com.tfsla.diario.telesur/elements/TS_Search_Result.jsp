...
<% if("Busqueda Avanzada".equals(submit)){%>
...
<div class="row clear">
	<div class="contS">
	<% if(pag == 1 ){ %>
		<div class="titlereg">Resultados de la búsqueda  <%--=q %> -- <%=t %> - <%=query %> -<%--=size %> - <%=query %> - <%=index %> - <%=pag %> - <%=result %> - <%=section_form --%></div>
	<% 	} %>	
		...
		<div class="ressearch" itemscope itemtype="http://schema.org/Article">
			...
			*/ --> 
			...
			<nt:news-list order="<%=order %>" size="<%=size%>" advancedfilter='<%=query %>' from="<%=published_form %>" page="<%=pag %>" section="<%=section_form %>" category="<%=filter_form %>" state="publicada">
						...
						<nt:news-list order="<%=order %>" size="<%=size%>" advancedfilter='<%=query %>' from="<%=published_form %>" page="<%=pag %>" section="<%=section_form %>" category="<%=filter_form %>" state="publicada">
					            ...
					            <c:if test="${hayMas == 'true'}" >
					                ...
					                <div class="contS" id="paginador" > 
					                    <a class="btnMas1" href="javascript:cargarMasNotas2('<%=pag + 1%>', '<%=size%>', '<%=q%>');">Ver más   </a>
					                </div>  
					            </c:if>
				        </nt:conditional-include>            
				   </nt:conditional-include>
			...
			<nt:news-list order="<%=order %>" size="<%=size%>" tags="<%=query%>" from="<%=published_form %>" page="<%=pag %>" section="<%=section_form %>" category="<%=filter_form %>" state="publicada">
				...
				<nt:conditional-include onposition="last">
					...
					<nt:news-list order="<%=order %>" size="<%=size%>" tags="<%=query%>" from="<%=published_form %>" page="<%=pag %>" section="<%=section_form %>" category="<%=filter_form %>" state="publicada">
				            ...
				            <c:if test="${hayMas == 'true'}" >
				                <div id="bloque_<%=pag + 1%>"></div>
				                <div class="contS" id="paginador" > 
				                    <a class="btnMas1" href="javascript:cargarMasNotas2('<%=pag + 1%>', '<%=size%>', '<%=t%>');">Ver más   </a>
				                </div>  
				            </c:if>
				        </nt:conditional-include>            
			    	</nt:conditional-include>
...
<form name="buscadorFooter" class="formSearchFooter" action="" method="post" style="display:none"> 
    ...
    <input class="subscribeSearchFooter" type="submit" value="Search" >
</form>
