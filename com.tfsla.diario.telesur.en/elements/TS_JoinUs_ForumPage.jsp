...
%>
<nt:news-list newstype="foros" size="<%=size%>" page="<%=pagina%>" order="user-modification-date desc" publication="9"> 
	<li>
		...
		<nt:conditional-include oncondition="${newslist.size == 10}">
			...
			<c:if test="${hayMas == 'true'}" >
				...
				<div class="forcenter" id="paginador">    
		                    <div class="forind">
		                        <div class="thread" onclick="cargarMasNotas('<%=pagina+1%>','<%=size%>');" style="cursor:pointer">See more</div>
		                    </div>
		                </div>
	                </c:if>
