...
if ( ErrorEdicion ) { 
	if ( passwordUpdateError ){ 
		msjAlert = "Error in password. Checks have written your current and new password correctly and matching.";
	}
	if ( ErrorNickNameIsAlreadyUsed ){ 
		msjAlert = "The entered nickname is invalid. Another user is using the same nickname, try to change it and try again.";
	} 
	if ( ErrorNickName ){
		msjAlert = "The entered nickname is invalid. Try to change it and try again.";
	} 
	if (!MsgEmtyFieldsF.equals("") ){
		msjAlert = "You must complete your name";
	} 
	if (!MsgEmtyFieldsL.equals("") ){
		msjAlert = "You must fill in your name.";
	} 
	if (FotoGrande) {
		msjAlert = "The photo you want to upload is too big";
	}
...
%>		 
			<div class="clear error conf">
				<div class="txtavisofondo"> Your profile is correctly associated with <%=request.getParameter("ASC") %>! </div> 
			</div>
...
%>		 
			<div class="clear error conf">
				<div class="txtavisofondo"> Your profile has been disconnected properly with <%=request.getParameter("DIS") %>! </div>
			</div>
...
%>		 
			<div class="clear error conf">
				<div class="txtavisofondo"> You have successfully changed your password. Now you can login as a user teleSUR! </div> 
			</div>
<%		}		
		if ( userUpdated && !errorUpdate && !passwordUpdateError && !ErrorNickNameIsAlreadyUsed && !ErrorNickName ) {
%>
			<div class="clear error conf">
				<div class="txtavisofondo"> Your profile has been edited successfully! </div> 
			</div>
...
%>		 
			<div class="error">
				<div class="txtaviso"> Failed to unlink google  <%=request.getParameter("NAP") %> account! </div> 
			</div>
<%		}		
%>		
		<!-- inicio col-mid -->
		<div class="col-mid">
			...
			<div class="cont">
				...
				<input type="text" placeholder="Name" name="firstname" value="<%=firstname%>" required <% if (!MsgEmtyFieldsF.equals("") ){ %> class="mal" <% } %>  >
				<input type="text" placeholder="Last name" name="lastname" value="<%=lastname%>" required <% if (!MsgEmtyFieldsL.equals("") ){ %> class="mal" <% } %>  >
				<input type="text" placeholder="User name" name="nickname" value="<%=nickname%>" <% if (ErrorNickNameIsAlreadyUsed || ErrorNickName )  { %> class="mal" <% } %> maxlength="30" required />
				...
				<div class="clear row">
					<div class="clear row">
						...
						<input type="file" name="picture" title="Browse" >
					</div>
...
%>			                        
			                        ...
			                        Choose a password teleSUR <br>
			                        to log on to your account <br>
			                        without using the <%= request.getSession().getAttribute("providerName") == null ? " social network " : (String)request.getSession().getAttribute("providerName")%> 
						</p>
						

			                       
			                        <a href="/english/users/selectkey.html" title="Select password">[Select password]</a>
<%
						} else { 
%>		                        	
						<p style="color:#666666;margin-right:1%; float:left; font-size:1.4em;"> Change password </p>
			                        <input type="password" <% if (passwordUpdateError) { %> class="mal" <% } %> name="password" placeholder="Password">
			                        
			                        <input type="password" <% if (passwordUpdateError) { %> class="mal" <% } %> name="password_new" placeholder="New password">
			                        <div class="anote">Must be 7 digits and contain numbers and letters.</div>

		                        	<input type="password" <% if (passwordUpdateError) { %> class="mal" <% } %> name="password_new_confirm" placeholder="Confirm new password">
<%
					  	}
					//  }
%>
		...
		<div class="col-mid">
			<div class="border_form" style="border-left:1px solid #CCC">
				<div class="cont">
					<div class="clear row">
							<p style="color:#666666;width:50%;margin-right:2%; margin-bottom:40px;float:left; font-size:1.2em; line-height:1.5em;"> 
							What name should be visible when you make a comment or post a report on Soy Reportero? 	
												</p>
						<select class="selreg" id="select" name="NameToShow" style="float:right; width:40%">
							<% if ( showName ) { %>
								<option value="showName"> With my real name </option>
								<option value="showNickname"> With my user name </option>
							<% } else { %>
								<option value="showNickname"> With my user name </option>
								<option value="showName"> With my real name </option>
					...
					<div class="clear row">
						<p style="color:#666666;width:50%;margin-right:2%; margin-bottom:40px;float:left; font-size:1.2em; line-height:1.5em;"> 
							Your e-mail should be public on your public profile? 
						</p>
					...
					<div class="clear row">
						<p style="color:#666666;margin-right:2%;margin-bottom:5px;float:left;font-size:1.2em;line-height:1.5em;"> 
							What notifications do you want to received?  
						</p>
						<div class="wpCheck">
							...
							<div class="txtregister">
								<label for="notifyOnPostCommentedCheck">
									When my report receives a comment
								</label>
							</div>
						...
						<div class="wpCheck">
							...
							<div class="txtregister">
								<label for="notifyOnCommentReplyCheck">
									When somebody replies to one of my comments.
								</label>
							</div>
						</div>
						<div class="wpCheck">
							...
							<div class="txtregister">
								<label for="notifyBoletinteleSURCheck"> News bulletin </label>
					...
					<div class="clear row" style="margin-bottom:25px">
						...
						<nt:providers>
							<nt:conditional-include oncondition="${provider.name == 'webusers-facebook' }">
								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] != 'true' }">
									<a href="javascript:openPopUpProvider('facebook', true, '<%=publicationId%>', '<%=publicationUrl%>' );"  title="Conect with Facebook" class="face">
										<cms:img src="/arte/asociar-facebook.png" style="width:106px" alt="facebook"/>
									</a>
								</nt:conditional-include>
								
								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] == 'true' }">
									<a href="javascript:disassociateProvider('facebook', '<%=publicationId%>');" title="Disconect with Facebook" class="face">
										<cms:img src="/arte/asociar-facebook.png" style="width:106px" alt="facebook"/>
									</a>
								</nt:conditional-include>
							</nt:conditional-include> 
							...
							<nt:conditional-include oncondition="${provider.name == 'webusers-twitter' }">
								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] != 'true' }">
									<a href="javascript:openPopUpProvider('twitter', true, '<%=publicationId%>', '<%=publicationUrl%>' );"  title="Conect with Twitter" class="twit">
										<cms:img src="/arte/asociar-twitter.png" style="width:106px" alt="twitter"/>
									</a>
								</nt:conditional-include>

								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] == 'true' }">
									<a href="javascript:disassociateProvider('twitter', '<%=publicationId%>');" title="Disconect with Twitter" class="twit">
										<cms:img src="/arte/asociar-twitter.png" style="width:106px" alt="twitter"/>
									</a>
								</nt:conditional-include> 
							</nt:conditional-include> 	
							...
							<nt:conditional-include oncondition="${provider.name == 'webusers-googlePlus' }">
								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] != 'true' }">
									<a href="javascript:openPopUpProvider('googlePlus', true, '<%=publicationId%>', '<%=publicationUrl%>' );"  title="Conect with google" class="goog">
										<cms:img src="/arte/asociar-google.png" style="width:106px" alt="google"/>
									</a>
								</nt:conditional-include> 
								<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] == 'true' }">
									<a href="javascript:disassociateProvider('googlePlus', '<%=publicationId%>');" title="Disconect with google" class="goog">
										<cms:img src="/arte/asociar-google.png" style="width:106px" alt="google"/> 
									</a>
								</nt:conditional-include> 
							</nt:conditional-include> 
						...
						<nt:providers>
							<nt:conditional-include oncondition="${ntuser.isassociatedto[provider.name] == 'true' }">
								<div class="anote"> You are associated with ${provider.description}. </div>
							</nt:conditional-include>
						</nt:providers>
						<div class="anote"> If you modified the profile data, save them before you attach or detach an account. </div>
					</div>

					<a id="cancelar" href="javascript:window.history.back(-1);" style="float:right">Cancel</a>

					<input id="subscribe" type="submit" value="Send" name="Send" style="float:right;margin-top:0">
				</div>
			</div>
		</div>
		<!-- fin col-midx -->
...
<script>
	function actualizarProflieHeader() {
		...
		var div = "#headerProfileAct";
		$.post("/system/modules/com.tfsla.diario.telesur.en/elements/webusers/TS_Profile_UserData.jsp",
			{ }, function(data){
			$(div).html(data);
