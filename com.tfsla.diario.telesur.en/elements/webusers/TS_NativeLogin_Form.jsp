...
%>
		...
		<% if(!errorDescription.equals("")){ %>
			<div class="cont">
				<div class="error">
				<% if (errorDescription.indexOf("El formulario ha sido completado incorrectamente. Verifique los campos resaltados") > -1 ) { %>
					<div class="txtaviso">The information provided is incomplete. Verify the highlighted fields.</div>
				<% } else { %>
		...
		<% if(loginError && request.getParameter("login_username") != null){ %>
			<div class="cont">
				<div class="error">
					<div class="txtaviso">User and / or password incorrect.</div>
				</div>
			</div>
			<div class="separador"></div>
		<% } %>
		<div class="row clear">
			<div class="col">
				<div class="cont titreg">Log in to teleSUR </div>
				...
				<div class="col-mid"><!-- inicio col-mid -->
					<div class="cont" style="height:340px">
						<form id="login" name="login" class="form-elements" method="post" action="<%= currentUri %>">
							<input <% if ( loginError && request.getParameter("login_username") != null ) { %>  class="mal" <% } %> type="text" id="login_username" name="login_username" value="" placeholder="Enter your e-mail. *" >
						        <input <% if ( loginError && request.getParameter("login_password") != null ) { %>  class="mal" <% } %> type="password" id="login_password"  name="login_password" placeholder="Enter your password *" />		
							...
							<div class="checkline row clear">   
								...
								<div class="txtregister" style="width:50%">Remember me</div>
						
					       		</div>    
					       		<p style="border-bottom: 1px solid #CCCCCC;float: left;margin-bottom: 25px;width: 100%;"></p> 
					    
					    		<a href="/english/users/password.html" class="olvideLogIn">Forgot your password?</a>
					       		<a href="/english/index.html" id="cancelar" style="float:right">Cancel</a>
					       		<input id="subscribe" type="submit" value="Submit" name="submit" style="float:right">
				       		</form>
			       		</div>
			    	</div>
			</div><!-- fin col-mid -->       
