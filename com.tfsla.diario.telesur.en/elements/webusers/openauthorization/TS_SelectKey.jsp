...
if(request.getParameter("accion") != null){
	...
	if (!errorConf && !errorVal && !processError && process)
		response.sendRedirect(request.getScheme() + "://" + request.getServerName() + "/english/users/profile_edit.html?CNP");
	
}
%>
	
	<div class="col-mid">
      		<div class="cont titreg"> Select Password </div>
		...
		<% if (errorVal) { %>	     	
	                <div class="cont"><!--ini content error-->
	                       <div class="error"><!--ini box error-->
	                               	<div class="txtaviso">
	                               		Password must be greater than 7 digits and contain numbers and letters.
	                         	</div>
	                       </div><!-- fin error -->
	                </div><!--fin cont-->
                <% } %>
		...
		<form class="reg-form" id="frmSubmit" name="frmSubmit" action="/english/users/selectkey.html" method="post">
			<input type="hidden" name="accion" value="seleccionarclave">
        		
        		<input type="password" placeholder="Enter you password *" id="password" name="password" required>*
        		<div class="anote">Password must be greater than 7 digits and contain numbers and letters.</div>
        		
        		<input type="password" placeholder="Repit you password * " id="confirm_password" name="confirm_password" required>*
		        
	            	<div class="clear"></div>
		    	<input type="submit" value="Select Key" id="seleccionarClave" style="width: 150px;">
		    	<div class="clear"></div>
		    	<div class="anote" style="margin-top:8px;">* Requested fields.</div>
		    	<a href="/english/users/selectkey.html" class="cancelForm">Cancel </a>
	     	</from>
	     	
	     	<p style="color:#666666;width:50%;margin-right:2%; float:left; font-size:1.2em; line-height:1.5em;">
			We will send an email to the address below. The use to login teleSUR.
          	</p>
