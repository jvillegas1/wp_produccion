...
boolean perfilPublico 	= (currentUri.indexOf("/english/users/profile.html") > -1 ) ? true : false ;
...
if(session.getAttribute("url") != null){
	...
	lastUrl = "/english/section/blog/index.html";
}
...
int totalNotas = 0; 
String link = "/english/staff/"+username;
String titulo = "TeleSUR Profile";
	...
	<div class="col-620 cont">			             	
		<div class="nworldtop cont">
			...
			<c:if test="${group == 'Bloggers_en' || group == 'Bloggers' }">		
				...
				<a href="<%=link%>" class="her" title="<%=titulo%>"><nt:user-firstname/> <nt:user-lastname/></a>							
		...
		<div class="right">
			<div >
				...
				<% if (usuarioLogueado.equals(username) && perfilPublico) { %>
					<a style="float:right; color:#666; font-size:1.4em; margin: 17px 0px 0px;" href="/english/users/profile_edit.html">
						Edit Profile
					</a>
				<% } %>
				<% if (usuarioLogueado.equals(username) && !perfilPublico) { %>
					<a style="float:right; color:#666; font-size:1.4em; margin: 17px 0px 0px;" href="/english<nt:user-link-friendly />">
						Public Profile
					</a>
				<% } %>
			</div>
			<div class="bottom">	
				<div class="blue_user_data">
					...
					<% if (totalNotas == 0 ) { %>
						<p style="font-size: 1.6em; margin: 0px;"> Not Recent posts </p>
					<% } %>
					
					<% if (totalNotas == 1 ) { %>
						<p style="font-size: 1.6em; margin: 0px;"> <%=totalNotas %> Recent post </p>
					<% } %>
					
					<% if (totalNotas > 1 ) { %>
						<p style="font-size: 1.6em; margin: 0px;"> <%=totalNotas %> Recent posts </p>
					<% } %>
				...
				<% if (usuarioLogueado.equals(username)) { %>
					<a href="/english/posts/post_story.html" title="Submit a story">
						<input type="submit" style="float: right;" id="subscribe" name="submit" value="Submit a story">
					</a>
					<a href="/english/posts/post_photo.html" title="Submit a photo">
						<input type="submit" style="float: right;" id="subscribe" name="submit" value="Submit a photo">
					</a>
					<a href="/english/posts/post_video.html" title="Submit a video">
						<input type="submit" style="float: right;" id="subscribe" name="submit" value="Submit a video">
					</a>
				<% } %>
			</div>
