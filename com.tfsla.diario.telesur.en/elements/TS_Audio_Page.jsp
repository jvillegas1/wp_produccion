...
*/
        ...
        * Orden de Busqueda
        */
        if ( display_search.equals("LAST") || display_search.equals("null") || display_search.equals("") ) display_search = "priority desc, user-modification-date desc";
...
%>
<div class="row clear">
    <div class="cont2"><h2 class="noResul" style="display:none" >No results were found in this category...</h2>
...
<c:set var="i" value="1"/>
<nt:news-list  order="user-modification-date desc" advancedfilter="<%=query %>" section="audio" author="<%=filter_autor%>" size="<%=size%>" page="<%=pagina%>" category="<%=filter_form%>"  publication="9"  >	
    ...
    <c:choose>
        <c:when test="${i % 2 == 0}">
            <ul class="list1">
                ...
                <li class="list1_desc"> <a class="her" href="<nt:link/>"><h4 itemprop="name headline"><nt:title value="detail"/></h4></a>
                ...
                <nt:conditional-include oncondition="${news.hideTime == 'false' || news.hideAuthor == 'false'}">
                    <div class="titsearch">
                        <nt:conditional-include oncondition="${news.hideTime == 'false'}">
                            <div class="autsearch"><fmt:setLocale value="en_US"/><fmt:formatDate pattern="d MMMM yyyy" value="${news.lastModificationDate}" /> / </div>
                        </nt:conditional-include>
                ...
                <div id="redes">
                    ...
                    <!--identificadores para el listado-->
                    ...
                    <c:set var="identificador1" value="${fn:substring(tmp5, 20, 35)}" />
                    <nt:audio>
                        <!--ventana modal para compartir en listado-->
                        <ul class="modal">
                            <li class="modaldeslizar">                                      
                                <a class="modal1" id="${identificador1}-share" onClick="openVentana(id);" >Share </a>
                            </li>
                            <li class="modaldeslizar"> 
                                <a href="<nt:audio-path/>" target="_blank" download="teleSUR-Audio.mp3"  class="btn btn-dowloand" role="button" title="Descarga"> Download</a>
                            </li>
                        </ul>
                        <div class="${identificador1}-share" id="ventana">
                            <div class="modalbox form">
                                <h1 class="sharetit">Share</h1>
			...
			get_short_url(long_url, login, api_key, function(short_url) {
			   document.getElementById('${identificador1}').value = '<iframe frameborder="0" height="180" scrolling="auto" src="http://www.telesurtv.net/seccion/audio/audio/index.html?audio=http://www.telesurtv.net<nt:audio-path/>&imagen='+short_url+'&titulo=${news.titles[news.titleDetailNum]}&category=${categoriDescription}" width="100%"></iframe>';
                ...
                <p class="no"><a href="#." id="${identificador1}" class="button-play"><img id="my_image" src="/arte/play.png" title="Play" width="35px" heith="35px" class="img-responsive" alt=""><span></span></a></p>
        ...
        <c:otherwise>
            <ul class="list2">
                ...
                <div id="redes2">                          
                    ...
                    <%--identificadores para lista de audio--%>
                    <nt:audio>
                        <c:set var="tmp1" value="${news.localpath}"/>
                        <c:set var="tmp2" value="${fn:toLowerCase(tmp1)}" />
                        <c:set var="tmp3" value="${fn:trim(tmp2)}" />
                        <c:set var="tmp4" value="${fn:replace(tmp3,'/', '-')}" />
                        <c:set var="tmp5" value="${fn:replace(tmp4,'noticia_', '')}" />
                        <c:set var="identificador2" value="${fn:substring(tmp5, 20, 35)}" />
                        <%--Variables de post para embed --%>

                        <ul class="modal">
                            <li class="modaldeslizar2"><a class="modal1"  id="${identificador2}-share" onClick="openVentana(id);" >Share</a></li>
                            <li class="modaldeslizar2"><a href="<nt:audio-path/>" target="_blank" download="teleSUR-Audio.mp3"  class="btn btn-dowloand btn-dowl2" role="button" title="Descarga"> Download</a></li>
                        </ul>
			...
			var login = "telesur";
			var api_key = "R_8925c37a76786f9a5620d8c3f0f9e65c";
			var long_url = "http://www.telesurtv.net${image2}";
			...
			get_short_url(long_url, login, api_key, function(short_url) {
                ...
                <script>
                    ...
                    playerInstance.setup({
                        file: "http://www.telesurtv.net<nt:audio-path/>",
                        width: "100%",
                        height: 30
                    });
    ...
    <nt:conditional-include onposition="last">	
        ...
        <nt:conditional-include oncondition="${ (newslist.size == 6 && newslist.page == 1) || (newslist.size == 6 &&  newslist.page != 1) }">	
            <nt:news-list page="<%=pagina+1%>" order="<%=display_search%>" advancedfilter="<%=query %>" section="audio" author="<%=filter_autor%>" size="<%=size%>" category="<%=filter_form%>"  publication="9" >
                <c:set var="hayMas" value="true"/>
            </nt:news-list>
            <c:if test="${hayMas == 'true'}" >
                <div id="bloque_<%=pagina+1%>"></div>
                <div class="cont" id="paginador" > 
                    <a class="btnMas1" href="javascript:cargarMasNotas2('<%=pagina+1%>','<%=size%>', '<%=filter_form%>');">See more  </a>
