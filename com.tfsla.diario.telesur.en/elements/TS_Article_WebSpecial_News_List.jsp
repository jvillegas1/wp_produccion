...
int position = 0;
String categoryPath = "/SubSecciones/en/multimedia/webspecial/"; 
%>
<div class="row clear">  
    <div class="col">
        <div class="rowInt">
            <nt:news-list order="user-modification-date desc" section="multimedia" page="<%=pagina%>" category="<%=categoryPath%>" size="<%=size%>" publication="9">
                ...
                <nt:conditional-include onposition="last">
                    ...
                    <nt:conditional-include oncondition="${newslist.size == 6}">						
                        <nt:news-list section="multimedia" size="<%=size%>" page="<%=pagina+1%>" category="<%=categoryPath%>" publication="9">						
                            <c:set var="hayMas" value="true"/>								
                        </nt:news-list>
                        <c:if test="${hayMas == 'true'}" >
                            <div id="bloque_<%=pagina+1%>"></div>
                            <div class="rowInt" id="paginador" > 
                                <a class="btnVermas clear" href="javascript:cargarMasNotas('<%=pagina+1%>','<%=size%>');">See More</a>
                            </div>
                        </c:if>	
                    </nt:conditional-include> 
                </nt:conditional-include> 
