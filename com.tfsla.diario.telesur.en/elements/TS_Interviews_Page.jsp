...
*/
	...
	session.setAttribute("seccion","analysis");
...
%>
	...
	<nt:news-list category="/SubSecciones/en/opinion/interviews" order="<%=display_search%>" advancedfilter="<%=query %>" author="<%=filter_autor%>" size="<%=size%>" page="<%=pagina%>" zone="normal" onmainpage="section" publication="9" searchindex="<%=index%>">
		...
		<nt:conditional-include onposition="last">
			...
			<nt:conditional-include oncondition="${newslist.size == 5}">	
				...
				<c:if test="${hayMas == 'true'}" >
					...
					<div class="cont" id="paginador" > 
						<a class="btnrepsm" href="javascript:cargarMasNotas('<%=pagina+1%>','<%=size%>','<%=display_search%>','<%=filter_autor%>','<%=buscar_search%>');"><img src="/arte/down-arrow.png" width="24" height="13" alt="" />View more analysis</a>
					</div>	
				</c:if>
			</nt:conditional-include> 			 
		</nt:conditional-include> 
	...
	<% if (pagina == 1) { %>
		...
		<br>
		<hr>
		<span style="color: rgb(255, 253, 253); font-family: &quot;Arimo&quot;,sans-serif; border-radius: 5px; text-align: center; line-height: 15px; margin: 70px; font-style: italic; background-color: rgb(26, 32, 66); padding: 5px; font-size: 12px;"> The opinions expressed in this section do not necessarily represent those of teleSUR </span>
		<br>
		<br>
		<hr>
	<% } %>
